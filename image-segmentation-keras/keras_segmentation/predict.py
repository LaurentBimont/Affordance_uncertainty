...
def predict_ensembliste(model, model_file=None, save_folder=None, inp=None, out_fname=None, checkpoints_path=None):
    '''
        Outputs
            pr : segmentation process throught argmax
            pr_pre : segmentation not process throught argmax
            unc : incertitude associée
        '''
    ...
    tirage = []
    for file in os.listdir('model_saved/' + save_folder):
        filename = os.fsdecode(file)
        model.load_weights('model_saved/' + save_folder + filename)
    ...
    unc = np.max(np.var(tirage, axis=0), axis=1)
    unc = unc.reshape((output_height,  output_width, 1))

    return pr, pr_pre, unc


def predict_bayesian(model=None, inp=None, out_fname=None, checkpoints_path=None):
    '''
    Outputs
        pr : segmentation process throught argmax
        pr_pre : segmentation not process throught argmax
    ...
    x = get_image_array(inp, input_width, input_height, ordering=IMAGE_ORDERING)
    tirage = np.array([model.predict(np.array([x]))[0] for i in range(50)])

    pr_pre = np.mean(tirage, axis=0)
    pr_pre = pr_pre.reshape((output_height, output_width, n_classes))
    pr = pr_pre.argmax(axis=2)
    unc = np.max(np.var(tirage, axis=0), axis=1)
    unc = unc.reshape((output_height,  output_width, 1))
